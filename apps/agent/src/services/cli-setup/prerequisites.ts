...
function buildServerClaudeMd(report: PrerequisitesReport): string {
  const installed = report.tools.filter((t) => t.status === 'installed')
  const lines: string[] = [
    '# Server Context (auto-generated by PocketDev)',
    '',
  ...
  for (const tool of installed) {
    ...
    lines.push(`- **${tool.name}**${version}${path}`)
  }
  ...
  if (report.databases.length > 0) {
    lines.push('', '## Detected Databases', '')
    for (const db of report.databases) {
      const v = db.version ? ` ${db.version}` : ''
      const cmd = DB_CONNECT_CMD[db.type]?.(db.port)
      lines.push(`- **${db.name}**${v} — ${db.status}, port ${db.port}`)
      if (cmd) lines.push(`  - Connect: \`${cmd}\``)
    }
  }

  const dataDir = process.env.POCKETDEV_DATA_DIR ?? join(process.cwd(), 'data')
  const dbPath = join(dataDir, 'pocketdev.db')
  lines.push(
    '',
    '## PocketDev Database',
    '',
    `SQLite at \`${dbPath}\` — query with: \`sqlite3 ${dbPath} "SELECT ..."\``,
    '',
    'Useful tables:',
    '- **tasks** — task history (id, prompt, agent_type, status, project_name, working_directory, created_at, completed_at)',
    '- **task_logs** — raw output lines per task (task_id, stream, line, timestamp)',
    '- **task_turns** — conversation turns (task_id, role, content)',
    '- **task_file_touches** — files modified per task (task_id, file_path, action)',
    '- **plans** / **plan_steps** — agent planning data (title, description, status, steps)',
    '- **projects** — known repositories (name, absolute_path, remote_url)',
  )
...
function writeServerClaudeMd(report: PrerequisitesReport): void {
  try {
    const home = process.env.HOME ?? process.env.USERPROFILE ?? '/root'
    const claudeDir = join(home, '.claude')
    mkdirSync(claudeDir, { recursive: true })
    writeFileSync(join(claudeDir, 'CLAUDE.md'), buildServerClaudeMd(report), 'utf-8')
  } catch {
    // Non-critical — silently skip if the write fails
  }
}
...
async function checkAllPrerequisitesUncached(): Promise<PrerequisitesReport> {
  ...
  const result: PrerequisitesReport = {
    ...osInfo,
    tools,
    databases,
