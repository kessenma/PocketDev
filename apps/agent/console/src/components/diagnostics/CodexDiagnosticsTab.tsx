          ...
          {codexInfo?.lastReplayDebug ? (
            <div className="mt-3 space-y-3">
              <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-3">
                <p className="text-xs uppercase tracking-[0.22em] text-foreground/38">Result</p>
                ...
                <p className="mt-1 text-sm text-foreground/70">{codexInfo.lastReplayDebug.error ?? 'No error reported'}</p>
              </div>
              <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-3">
                <p className="text-xs uppercase tracking-[0.22em] text-foreground/38">Input Callback URL</p>
                <p className="mt-2 break-all font-mono text-xs text-text-terminal">{codexInfo.lastReplayDebug.inputCallbackUrl ?? 'None'}</p>
                <p className="mt-2 text-xs text-foreground/45">{new Date(codexInfo.lastReplayDebug.recordedAt).toLocaleString()}</p>
              </div>
            </div>
          ) : (
            <div className="mt-3 rounded-[1.2rem] border border-dashed border-border/50 bg-background/30 p-4 text-sm text-foreground/52">
          ...
          {codexInfo?.lastReplayDebug?.attempts.length ? (
            <div className="space-y-2">
              {codexInfo.lastReplayDebug.attempts.map((attempt, index) => (
                <div key={`${attempt}-${index}`} className="rounded-xl border border-border/40 bg-background/40 p-3 font-mono text-xs text-text-terminal">
                  {attempt}
                </div>
              ))}
            </div>
          ...
          <div className="min-h-0 overflow-y-auto rounded-[1.5rem] border border-border/40 bg-background p-3">
            ...
            <div className="mt-3 space-y-3">
              <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-4">
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-foreground/38">Last Replay Prompt</p>
                <p className="mt-2 text-sm text-foreground/80">{codexInfo?.lastReplayDebug?.sessionPrompt ?? 'None'}</p>
              </div>
              <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-4">
