...
const ORIGIN_STYLES: Record<string, { label: string; className: string }> = {
  app: { label: 'App', className: 'border-primary/40 text-primary' },
  task: { label: 'AI Task', className: 'border-green-500/40 text-green-400' },
  external: { label: 'External', className: 'border-border/50 text-foreground/50' },
}
...
function CommitRow({ commit }: { commit: GitHistoryDebugCommit }) {
  ...
  return (
    <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-3">
      <button
        type="button"
      ...
      >
        {expanded
          ? <ChevronDown className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" />
          : <ChevronRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" />}
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="shrink-0 font-mono text-xs text-[var(--bauhaus-yellow)]">{commit.sha}</span>
          ...
          <p className="mt-1 truncate text-sm text-foreground/85">{commit.message}</p>
      ...
      {expanded && commit.files.length > 0 && (
        <div className="mt-3 space-y-1 border-t border-border/25 pt-3">
          {commit.authorEmail && (
            <p className="mb-2 font-mono text-[11px] text-foreground/35">{commit.authorEmail}</p>
          )}
          {commit.branch && (
            <p className="mb-2 text-[11px] text-foreground/45">
              Branch: <span className="font-mono text-primary">{commit.branch}</span>
            </p>
          )}
          {commit.files.map((file, i) => (
            <div key={`${file.path}-${i}`} className="flex items-center gap-2 text-xs">
              <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                file.kind === 'added' ? 'bg-green-400'
                : file.kind === 'deleted' ? 'bg-red-400'
                : file.kind === 'renamed' ? 'bg-primary'
                : 'bg-[var(--bauhaus-yellow)]'
              }`} />
              <span className="min-w-0 flex-1 truncate font-mono text-foreground/65">
                {file.path}
    ...
    <div className="grid h-full gap-3 xl:grid-cols-[minmax(320px,0.78fr)_minmax(0,1.22fr)]">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-[1.5rem] border border-border/40 bg-background p-4">
          ...
          <div className="mt-3 space-y-3">
            <div className="rounded-[1.2rem] border border-border/40 bg-background/40 p-3 text-sm text-foreground/78">
        ...
        <div className="rounded-[1.5rem] border border-border/40 bg-background p-4">
          <div className="flex items-center gap-2">
            <GitCommitHorizontal className="h-4 w-4 text-[var(--bauhaus-yellow)]" />
          ...
          <div className="mt-3 rounded-[1.2rem] border border-border/40 bg-background/40 p-3 text-sm text-foreground/78">
            <p>Project: <span className="font-mono text-text-terminal">{gitHistoryInfo?.projectId ?? 'None'}</span></p>
            {gitHistoryInfo?.syncStatus ? (
              <>
                <p>HEAD: <span className="font-mono text-text-terminal">{gitHistoryInfo.syncStatus.headSha.slice(0, 10)}</span></p>
                <p>Last synced: <span className="font-mono text-text-terminal">{gitHistoryInfo.syncStatus.lastSyncedSha?.slice(0, 10) ?? 'Never'}</span></p>
                <p>Pending commits: {gitHistoryInfo.syncStatus.pendingCommits}</p>
              </>
            ) : (
              <p className="text-foreground/45">No sync status available</p>
            )}
            <p>Commits in DB: {gitHistoryInfo?.commits.length ?? 0}{gitHistoryInfo?.hasMore ? '+' : ''}</p>
            {gitHistoryInfo?.syncError && (
              <p className="mt-2 text-red-400">Error: {gitHistoryInfo.syncError}</p>
            )}
          </div>
        </div>
        <div className="rounded-[1.5rem] border border-border/40 bg-background p-4">
          ...
          <div className="mt-3 space-y-2">
            {offlineSnapshots.length > 0 ? (
              offlineSnapshots.map((snap) => (
                <div key={snap.id} className="rounded-[1.2rem] border border-border/40 bg-background/40 p-3 text-sm text-foreground/78">
                  <p className="font-medium text-foreground/90">{snap.deviceName ?? snap.deviceId.slice(0, 12)}</p>
                  <p className="font-mono text-xs text-text-terminal">{snap.branch}</p>
              ...
              <p className="text-xs text-foreground/45">No devices have offline snapshots.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid min-h-0 gap-3 lg:grid-cols-2">
        ...
        <div className="min-h-0 space-y-3 overflow-y-auto">
          <div className="rounded-[1.5rem] border border-border/40 bg-background p-3">
            ...
            <div className="mt-3 space-y-2">
              {projectsInfo?.recentOperations.length ? (
                projectsInfo.recentOperations.map((entry, index) => (
                  <div key={`${entry.ts}-${index}`} className="rounded-xl border border-border/40 bg-background/30 p-3">
                    <div className="text-[10px] uppercase tracking-[0.22em] text-foreground/35">
                      {formatShortTime(entry.ts)} · {entry.kind}
                    </div>
                    <div className="mt-1 break-words font-mono text-xs text-text-terminal">{entry.message}</div>
                  </div>
                ))
              ) : (
                <div className="text-xs text-foreground/58">No project operations recorded yet.</div>
              )}
            </div>
          </div>
        </div>
