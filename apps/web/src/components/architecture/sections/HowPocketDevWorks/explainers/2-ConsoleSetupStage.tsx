...
export function ConsoleSetupStage({
  ...
  // --- Timeline segments ---
  const zoomP = easeOut(mapProgress(p, 0.0, 0.12))
  const sshP = mapProgress(p, 0.10, 0.22)
  const curlP = mapProgress(p, 0.22, 0.30)
  const installP = mapProgress(p, 0.28, 0.44)
  const urlP = mapProgress(p, 0.44, 0.54)
  const morphP = mapProgress(p, 0.54, 0.64)
  const formFillP = mapProgress(p, 0.62, 0.76)
  const tapP = mapProgress(p, 0.76, 0.84)
  const dashboardP = mapProgress(p, 0.84, 1.0)
  ...
  // --- Blue circle behind laptop (top-left corner, like a badge) ---
  // In viewBox coords: top-left of the laptop screen lid
  const screenTopY = laptopCy - 125 * laptopScale
  const screenLeftX = laptopCx - 90 * laptopScale
  const circleCx = screenLeftX + 12 * laptopScale
  const circleCy = screenTopY + 18 * laptopScale
  const circleR = mix(10, 38, zoomP)
  ...
  const settled = dashboardP > 0.96

  return (
    <>
      {/* Blue circle — top-left behind the laptop like a badge, gentle hover */}
      <motion.circle
        cx={circleCx}
        cy={circleCy}
        r={circleR}
        ...
        animate={
          active && !reduceMotion && zoomP >= 1
            ? {
                cy: [circleCy, circleCy - 5, circleCy + 3, circleCy],
                scale: [1, 1.06, 0.97, 1],
              }
            : { scale: 1 }
        }
          ...
          {showTap && (
            <>
              <motion.circle
                ...
                r={8}
                fill={palette.bauhaus.black}
                animate={{ opacity: tapP > 0.3 ? 0.9 : 0 }}
      ...
      {showDashboard && (
        <g opacity={dashboardP}>
          ...
          {/* Mini QR grid — hint at ConnectStage */}
          {[0, 1, 2, 3, 4].map((row) =>
            [0, 1, 2, 3, 4].map((col) => {
              const filled = (row + col) % 2 === 0 || (row === 2 && col === 2) || (row === 0 && col < 3) || (row === 4 && col > 1)
                ...
                <motion.rect
                  ...
                  animate={
                    settled && active && !reduceMotion
                      ...
                      : { opacity: dashboardP * 0.85 }
                  }
                  transition={
                    settled && active && !reduceMotion
                      ? { duration: EXPLAINER_TIMINGS.long, repeat: Infinity, ease: 'easeInOut', delay: (row + col) * 0.06 }
                      : { duration: 0.2 }
                  }
                />
              )
