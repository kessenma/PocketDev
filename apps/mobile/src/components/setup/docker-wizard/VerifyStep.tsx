...
export default function VerifyStep({ dispatch }: Props) {
  ...
  const [composeVersion, setComposeVersion] = useState<string | null>(null)
  ...
  async function handleVerify() {
    ...
    try {
      const result = await postVerifyDocker(server.ip, server.port)
      setVersion(result.version)
      setPath(result.path)
      setComposeVersion(result.compose_version)
      setDaemonRunning(result.daemon_running)

      if (result.installed && result.daemon_running) {
        setState('success')
        setTimeout(() => {
        ...
        if (!result.installed) missing.push('Docker')
        if (!result.daemon_running) missing.push('Docker daemon')
        if (!result.has_compose) missing.push('Docker Compose')
        setError(`Not ready: ${missing.join(', ')}. Try going back and re-running the previous steps.`)
      }
    } catch (err) {
      setState('failed')
    ...
    <View style={styles.container}>
      <View style={styles.center}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          {state === 'success' ? 'Docker Engine and Compose are installed and running.' :
            state === 'loading' ? 'Checking Docker, daemon, and Compose...' :
            state === 'failed' ? 'Some components could not be verified.' :
            'Confirm that Docker Engine, daemon, and Compose are working.'}
        </Text>

        {state === 'success' && (
          <View style={[styles.detailsCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <DetailRow label="Docker" value={version ? `v${version}` : 'installed'} ok colors={colors} />
            <DetailRow label="Path" value={path ?? 'unknown'} ok colors={colors} />
            <DetailRow label="Daemon" value={daemonRunning ? 'running' : 'stopped'} ok={daemonRunning} colors={colors} />
            <DetailRow label="Compose" value={composeVersion ? `v${composeVersion}` : 'available'} ok={!!composeVersion} colors={colors} />
          </View>
        )}
