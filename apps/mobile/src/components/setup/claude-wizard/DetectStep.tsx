...
export default function DetectStep({ dispatch }: Props) {
  ...
  const [statusResult, setStatusResult] = useState<ClaudeSetupStatus | null>(null)
  ...
  useEffect(() => {
    if (animationDone && statusResult) {
      dispatch({ type: 'DETECTION_COMPLETE', claudeStatus: statusResult })
    }
  ...
  async function detect() {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to check Claude CLI status'
      setError(message)
  ...
  return (
    <View style={styles.container}>
      <ClaudeSetupAnimation onComplete={() => setAnimationDone(true)} />
    </View>
  )
}
