...
import { postVerifyClaudeAuth } from '../../../services/api'
...
export default function VerifyStep({ dispatch }: Props) {
  ...
  async function handleVerify() {
    ...
    try {
      const result = await postVerifyClaudeAuth(server.ip, server.port)
    ...
    <View style={styles.container}>
      <View style={styles.center}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          {state === 'success' ? `Claude CLI v${version ?? 'unknown'} is ready` :
            state === 'loading' ? 'Checking your authentication status...' :
            state === 'failed' ? 'Authentication could not be verified' :
            'Confirm that Claude CLI is signed in to your account'}
        </Text>
