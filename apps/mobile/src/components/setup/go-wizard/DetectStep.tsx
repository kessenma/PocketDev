...
export default function DetectStep({ dispatch }: Props) {
  ...
  const [statusResult, setStatusResult] = useState<GoSetupStatus | null>(null)

  const detect = useCallback(async () => {
    if (!server) return
    setError(null)
    try {
      const status = await fetchGoSetupStatus(server.ip, server.port)
      setStatusResult(status)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to check Go status'
      setError(message)
  ...
  useEffect(() => {
    if (statusResult) {
      dispatch({ type: 'DETECTION_COMPLETE', goStatus: statusResult })
    }
  ...
  return (
    <View style={styles.center}>
      ...
      <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Checking Go setup...</Text>
    </View>
  )
}
